<#
.SYNOPSIS
Reports the open accounts payable (leverantörsreskontra) with an aging breakdown.

.DESCRIPTION
Returns the outstanding supplier invoices — those that have been posted but are
...
which is useful for planning upcoming payments.

The sum of the outstanding amounts reconciles against the balance of the payable
account (2440) in the general ledger.
...
Optional. If specified, only invoices for this supplier are included.
...
.EXAMPLE
Get-LedgerAccountsPayable -JournalPath .\MinFirma.ledger

Lists every open supplier invoice with its remaining amount, days overdue and
aging bucket.
...
function Get-LedgerAccountsPayable {
    ...
    $open = @(Get-LedgerSupplierInvoice -JournalPath $JournalPath -Unpaid)
    if ($PSBoundParameters.ContainsKey('SupplierNumber')) {
        $open = @($open | Where-Object { $_.SupplierNumber -eq $SupplierNumber })
    }
